...
public class CopycatAutomaticCouplerRenderer extends SmartBlockEntityRenderer<CopycatAutomaticCouplerBlockEntity> {
	...
	public CopycatAutomaticCouplerRenderer(BlockEntityRendererProvider.Context context) {
		super(context);
	}
	...
	protected void renderSafe(CopycatAutomaticCouplerBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay) {
		...
		if(!VisualizationManager.supportsVisualization(level)) {
			...
			translate(couplerOffset).
			rotate(couplerRot).
			translate(be.getCouplerLength(), 0, 0).
			color(be.color).
			light(light).overlay(overlay).
			renderInto(poseStack, vb);
		}
