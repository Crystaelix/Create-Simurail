...
public class PhysicsBogeyWheelSparkParticle extends TextureSheetParticle implements ParticleSubLevelKickable {
	...
	protected PhysicsBogeyWheelSparkParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, SpriteSet sprites) {
		super(level, x, y, z);
		this.xd = xd;
		this.yd = yd;
		this.zd = zd;
	...
	public void tick() {
		...
		setColor(1, Mth.lerp(age, 0.95F, 0.3F), Mth.lerp(age * age, 0.75F, 0.05F));
		alpha = 1 - age * age;
	}
	...
	public void render(VertexConsumer buffer, Camera camera, float partialTick) {
		...
		if(side.lengthSquared() < SimurailMathf.EPSILON) {
	...
	public record Provider(SpriteSet sprites) implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType options, ClientLevel level, double x, double y, double z, double xd, double yd, double zd) {
			return new PhysicsBogeyWheelSparkParticle(level, x, y, z, xd, yd, zd, sprites);
		}
	}
}
