...
import com.crystaelix.simurail.content.SimurailParticleTypes;
...
public class PhysicsBogeyEffects {
	...
	public static final float RAIL_CLEARANCE = 0.03125F;
	...
	protected final Vector3f along = new Vector3f();
	protected final Vector3f up = new Vector3f();
	protected final Vector3f lateral = new Vector3f();
	...
	public void tick() {
		...
		if(Math.abs(slipSpeed) <= SLIP_DEADZONE || !config.wheelSlipSparks.get()) {
	...
	protected void updatePivotBasis() {
		...
		pivotRot.transform(along.set(1, 0, 0));
		pivotRot.transform(up.set(0, bogey.isInverted() ? -1 : 1, 0));
		pivotRot.transform(lateral.set(0, 0, 1));
	}
	...
	protected float axlePosition(double[] axlePositions, int axle, int axleCount) {
		...
		return (float)bogey.options.type.axleSpacing() * ((float)axle / (axleCount - 1) - 0.5F);
	}

	protected void spawnSpark(Level level, RandomSource random, double slipSpeed, float axlePos, boolean left) {
		contactPoint(random, axlePos, left, contact);
		scrubVelocity(random, slipSpeed, velocity);

		level.addParticle(SimurailParticleTypes.WHEEL_SPARK.get(),
	...
	protected void contactPoint(RandomSource random, float axlePos, boolean left, Vector3f dest) {
		...
		float halfGauge = (float)(type.wheelGauge() * 0.5);

		float wheelReach = (float)(type.wheelCenterHeight() - type.wheelOuterRadius()) + RAIL_CLEARANCE;
		float trackNode = inverted ? 0.5F + axleOffset : -1.5F - axleOffset;

		dest.set(
				axlePos + jitter(random, CONTACT_JITTER_ALONG),
				inverted ? trackNode - wheelReach : trackNode + wheelReach,
				(left ? halfGauge : -halfGauge) + jitter(random, CONTACT_JITTER_ACROSS));
		pivotRot.transform(dest).add(pivotOffset);
	}
	
	protected void scrubVelocity(RandomSource random, double slipSpeed, Vector3f dest) {
		double scrub = -slipSpeed * 0.05;
		dest.zero();
		dest.fma((float)(scrub * (0.5 + random.nextDouble() * 0.7)), along);
		dest.fma((float)(Math.abs(scrub) * (0.1 + random.nextDouble() * 0.35)), up);
		dest.fma((float)(Math.abs(scrub) * (random.nextDouble() - 0.5) * 0.4), lateral);
	}
